  ...
  <div class="layout">
    ...
    <div class="main">
      <header class="header">
        ...
        <div class="breadcrumb"><span class="breadcrumb-item">P.B.S</span><span class="breadcrumb-sep">/</span><span class="breadcrumb-item current">성경 콘텐츠 목록</span></div>
      ...
      <div class="content">
        <div class="page-header-row">
          <div><div class="page-title">성경 콘텐츠 목록</div><div class="page-desc">등록된 성경 공부 및 묵상 콘텐츠 목록을 관리합니다.</div></div>
        ...
        <div class="card">
          <div class="filter-bar">
            <div class="filter-group">
              <select v-model="filterBook" style="min-width: 120px"><option value="">전체 성경</option><option value="창세기">창세기</option><option value="출애굽기">출애굽기</option><option value="마태복음">마태복음</option><option value="요한복음">요한복음</option></select>
              <select v-model="filterStatus" style="min-width: 110px"><option value="">전체 상태</option><option value="published">게시됨</option><option value="draft">임시저장</option></select>
            </div>
            <div class="filter-group" style="flex: 1"><input type="text" v-model="searchText" placeholder="제목 또는 묵상 내용 검색..." style="width: 100%" /></div>
            <div class="filter-group">
              <button class="btn btn-secondary" @click="fetchPbs"><svg style="width:15px;height:15px;stroke:currentColor;fill:none;stroke-width:2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>검색</button>
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="pb in pbs" :key="pb.id" @click="goToDetail(pb.id)">
                  ...
                  <td>{{ pb.author }}</td>
                  <td>{{ pb.date }}</td>
                  <td style="text-align: center"><span class="status-tag" :class="pb.status">{{ statusLabel(pb.status) }}</span></td>
                  <td style="text-align: center"><button class="btn-icon-only" title="수정" @click.stop="goToDetail(pb.id)"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button></td>
                </tr>
                <tr v-if="pbs.length === 0"><td colspan="7" style="text-align: center; color: var(--text-muted); padding: 40px">{{ loading ? '불러오는 중...' : '등록된 콘텐츠가 없습니다.' }}</td></tr>
              </tbody>
            </table>
          </div>
          <div class="pagination-container">
            <div class="pagination-info">전체 <strong>{{ pbs.length }}</strong> 개 항목 중 1 - {{ pbs.length }} 표시</div>
...
function statusLabel(status: string): string {
  const labels: Record<string, string> = { published: '게시됨', draft: '임시저장', archived: '보관됨' }
